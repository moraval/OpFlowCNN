...
--    layer:add(nn.HardTanh(-1,1))
  ...
  local function negConv(nIn, nOut, k, s, p)
    local layer = nn.Sequential()
    layer:add(nn.SpatialConvolution(nIn,nOut,k,k,s,s,p,p))
    layer = require('weight-init.lua')(layer, method)
    layer:add(nn.SpatialBatchNormalization(nOut))
...
--    model:add(conv(L1_chan, 2, 1, 1, 0))
    ...
    model:add(conv(channels*2, L1_chan, 7, 2, 3))
    model:add(conv(L1_chan, L2_chan, 3, 1, 1))
    model:add(conv(L2_chan, L2_chan, 3, 1, 1))
--    model:add(conv(L3_chan, L2_chan, 3, 1, 1))
--    model:add(conv(L3_chan, L4_chan, 3, 1, 1))
--    model:add(conv(L4_chan, L2_chan, 3, 1, 1))
    model:add(conv(L2_chan, L1_chan, 3, 1, 1))
    model:add(conv(L1_chan, 2, 1, 1, 0))
--    model:add(nn.Tanh())
--    model:add(conv(L3_chan, L2_chan, 5, 1, 2))
