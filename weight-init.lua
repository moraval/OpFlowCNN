...
-- Getting start flow under 1
local function w_my(fan_in, fan_out)
  return torch.uniform(-0.001,0.001)
end

local function w_approx_flow()
  return torch.uniform(-0.1,0.1)
end
...
local function w_init(net, arg)
  ...
  print('initializing')
  local method = nil
  if arg == 'approx_flow' then method = w_approx_flow
  elseif     arg == 'flow'    then method = w_my
  elseif     arg == 'heuristic'    then method = w_init_heuristic
  ...
  for i = 1, #net.modules do
    ...
    if m.__typename == 'nn.SpatialConvolution' then
      ...
      print('gacja nn')
    elseif m.__typename == 'cunn.SpatialConvolution' then
      m:reset(method(m.nInputPlane*m.kH*m.kW, m.nOutputPlane*m.kH*m.kW))
      print('gacja')
--      elseif m.__typename == 'cunn.VolumetricConvolution' then
--         m:reset(method(m.nInputPlane*m.kH*m.kW, m.nOutputPlane*m.kH*m.kW))
--         print('gacja')
