...
local nrOfBatches = trainData:size(1)/batchSize end
...
-- CRITERION
  ...
  local criterion = nn.OpticalFlowCriterion(namedir, printF, a, normalize, GT, batchSize)
----------------------------------------------------------------------
...
--      annealing of alfa
    ...
    if (epochs % print_freq == 0) then
      criterion = nn.OpticalFlowCriterion(namedir, printF, a, normalize, GT, batchSize)
    end
  end
